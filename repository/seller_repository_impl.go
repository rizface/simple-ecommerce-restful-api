...
func (s sellerRepositoryImpl) Register(ctx context.Context, tx *sql.Tx, request web.RequestSellerRegister) int64 {
...
func (s sellerRepositoryImpl) FindByEmail(ctx context.Context, tx *sql.Tx, email string) domain.Seller {
	sql := "SELECT id,nama_toko,email,alamat_toko,deskripsi, DATE_FORMAT(created_at, '%w %M %Y') FROM sellers WHERE email = ?"
	rows,err := tx.QueryContext(ctx,sql,email)
	helper.PanicIfError(err)
	defer rows.Close()
	seller := domain.Seller{}
	if rows.Next() {
		err := rows.Scan(&seller.Id,&seller.NamaToko,&seller.Email,&seller.AlamatToko,&seller.Deskripsi,&seller.CreatedAt)
		helper.PanicIfError(err)
	}
	return seller
}
...
func (s sellerRepositoryImpl) FindByName(ctx context.Context, tx *sql.Tx, name string) domain.Seller {
	sql := "SELECT id,nama_toko,email,alamat_toko,deskripsi, DATE_FORMAT(created_at, '%w %M %Y') FROM sellers WHERE nama_toko = ?"
